...
void IRAM_ATTR fsm_Mashine()
{
	...
	current->bComplete = 0;
	 ...
	 {
		Request(T_SETUP,ZERO_USB_ADDRESS,0b0000,T_DATA0,0x80,0x6,0x0100,0x0000,0x0012,0x0012); 	// 请求设备描述符
		current->fsm_state    = 5; 
	 }
	 else if(current->fsm_state==5)
	 {
		...
		{
			memcpy(&current->desc,current->acc_decoded_resp,0x12);	// 解析设备描述符
...
#if 1
		Request(T_SETUP,ZERO_USB_ADDRESS,0b0000,T_DATA0,0x00,0x5,0x0000+ASSIGNED_USB_ADDRESS,0x0000,0x0000,0x0000);	// 设置设备地址
		current->fsm_state    =  6; 
#else
		current->fsm_state    =  0;
#endif
	 ...
	 {
		Request(T_SETUP,ASSIGNED_USB_ADDRESS,0b0000,T_DATA0,0x80,0x6,0x0200,0x0000,0x0009,0x0009); 	// 请求配置描述符
		current->fsm_state    = 8; 
	 }
	 else if(current->fsm_state==8)
	 {
		if(current->acc_decoded_resp_counter==0x9)
		{
			...
			Request(T_SETUP,ASSIGNED_USB_ADDRESS,0b0000,T_DATA0,0x80,0x6,0x0200,0x0000,current->cfg.wLength,current->cfg.wLength);	// 请求完整配置描述符
			current->fsm_state    = 9;
		}
		else
		{
	 ...
	 {
		if(current->acc_decoded_resp_counter==current->cfg.wLength)	// 检查配置描述符长度
		{
	 ...
	 else if(current->fsm_state==97)
	 {
		...
		Request(T_SETUP,ASSIGNED_USB_ADDRESS,0b0000,T_DATA0,0x00,0x9,0x0001,0x0000,0x0000,0x0000);	// SET_CONFIGURATION

		current->hid_report_desc_count = 0;
	 ...
	 {
		// config interfaces??
		Request(T_SETUP,ASSIGNED_USB_ADDRESS,0b0000,T_DATA0,0x21,0xa,0x0000,0x0000,0x0000,0x0000);	// HID SET_IDLE
	 ...
	 else if(current->fsm_state==99)
	 {
		...
		//uint8_t cmd1 = current->cnt&0x20?0x7:0x0;
		 ...
		 if(current->flags_new!=current->flags)
		 {
			...
			RequestSend(T_SETUP,ASSIGNED_USB_ADDRESS,0b0000,T_DATA0,0x21,0x9,0x0200,0x0000,0x0001,0x0001,&current->flags);	// HID SET_REPORT
		 }
		//if(cmd0!=cmd1)
		//{
...
static int cntl = 0;		
		...
		if(pcurrent->ufPrintDesc&32){
			/*
			pcurrent->ufPrintDesc &= ~(uint32_t)32;
			char title_buffer[50];
			for(int i=0;i<pcurrent->hid_count;i++){
				sprintf(title_buffer, "HID报告描述符 %d :", i);
				print_hex_dump(title_buffer, pcurrent->hid_report_desc_buffer[0], pcurrent->hid[0].wDescriptorLength);
			}*/
		}
}
